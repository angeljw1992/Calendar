        ...
        <div class="card-body">
            <form method="POST" action="{{ route('admin.tasks.update', [$task->id]) }}" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <select class="form-control sl2 {{ $errors->has('name') ? 'is-invalid' : '' }}" name="name"
                        id="name">
                        <option value disabled {{ old('name', null) === null ? 'selected' : '' }}>
                            {{ trans('global.pleaseSelect') }}</option>
                        @foreach (App\Models\Task::NAME_SELECT as $key => $label)
                            <option value="{{ $label }}"
                                {{ old('name', $task->name) === (string) $label ? 'selected' : '' }}>{{ $label }}
                            </option>
                        @endforeach
                    </select>
                    @if ($errors->has('name'))
                ...
                <div class="form-group">
                    ...
                    <select class="form-control {{ $errors->has('grupo') ? 'is-invalid' : '' }}" name="grupo"
                        id="grupo" required>
                        <option value disabled {{ old('grupo', null) === null ? 'selected' : '' }}>
                            {{ trans('global.pleaseSelect') }}</option>
                        @foreach (App\Models\Task::GRUPO_SELECT as $key => $label)
                            <option value="{{ $key }}"
                                {{ old('grupo', $task->grupo) === (string) $key ? 'selected' : '' }}>{{ $label }}
                            </option>
                        @endforeach
                    </select>
                    @if ($errors->has('grupo'))
                ...
                <div class="form-group">
                    ...
                    <input class="form-control date {{ $errors->has('due_date') ? 'is-invalid' : '' }}" type="text"
                        name="due_date" id="due_date" value="{{ old('due_date', $task->due_date) }}" required>
                ...
                <div class="form-group">
                    ...
                    <input class="form-control date {{ $errors->has('final_date') ? 'is-invalid' : '' }}" type="text"
                        name="final_date" id="final_date" value="{{ old('final_date', $task->final_date) }}" required>
                ...
                <div class="form-group">
                    ...
                    <input class="form-control timepicker {{ $errors->has('start') ? 'is-invalid' : '' }}" type="text"
                        name="start" id="start" value="{{ old('start', $task->start) }}" required>
                    @if ($errors->has('start'))
                ...
                <div class="form-group">
                    ...
                    <input class="form-control timepicker {{ $errors->has('end') ? 'is-invalid' : '' }}" type="text"
                        name="end" id="end" value="{{ old('end', $task->end) }}" required>
                    @if ($errors->has('end'))
                ...
                <div class="form-group">
                    ...
                    <select class="form-control {{ $errors->has('dias') ? 'is-invalid' : '' }}" name="dias"
                        id="dias" required>
                        <option value disabled {{ old('dias', null) === null ? 'selected' : '' }}>
                            {{ trans('global.pleaseSelect') }}</option>
                        @foreach (App\Models\Task::DIAS_SELECT as $key => $label)
                            <option value="{{ $key }}"
                                {{ old('dias', $task->dias) === (string) $key ? 'selected' : '' }}>{{ $label }}
                            </option>
                        @endforeach
                    </select>
                    @if ($errors->has('dias'))
